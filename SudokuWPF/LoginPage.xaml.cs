...
namespace SudokuWPF
{
    ...
    public partial class LoginPage : Page
    {
        ...
        private void log_in_click(object sender, RoutedEventArgs e)
        {
            if (Check_Fields())
            {
                string userPassword = _userCredentialsRepository.GetUserPassByLogin(TextboxLogin.Text);
                if (userPassword != null)
                {
                    if (VerifyHashedPassword(userPassword, PasswordBox.Password))
                    {                                                                       
                        ((_authorizationWindow.Owner as MainWindow).DataContext as SudokuModel).AuthUser = _userCredentialsRepository.GetUserByCredentials(TextboxLogin.Text, userPassword);
